...
function ArticleCard({ article, showExcerpt }: { article: NewsArticle; showExcerpt?: boolean }) {
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm hover:border-[#FFD23F] hover:shadow-md transition-all">
      <Link href={href} className="relative aspect-[16/10] overflow-hidden block">
        ...
        <span className="absolute top-3 left-3 rounded-full bg-[#FFD23F] px-2.5 py-0.5 text-[10px] font-bold uppercase text-[#111111]">
          Article
        </span>
      </Link>
      <div className="flex flex-1 flex-col p-4 sm:p-5">
        ...
        {showExcerpt !== false && (
          <p className="text-sm text-gray-600 line-clamp-3 mb-4 flex-1">
            {article.description || 'Read the full story for tips, deals, and savings advice.'}
          </p>
        )}
        <Link
          href={href}
  ...
  const displayArticles =
    articles.length > 0
      ...
      : Array.from({ length: 8 }, (_, i) => ({
          ...
          title: 'Latest post coming soon',
          description: 'Our editors are preparing new savings guides and deal roundups.',
          imageUrl: '',
          createdAt: new Date().toISOString(),
        }));

  const visible = displayArticles.slice(0, visibleCount);
  ...
  return (
    <section className="bg-white py-10 sm:py-12 border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-8">Latest Posts</h2>
            ...
            {hasMore && (
              <div className="mt-10 text-center">
                <button
                  ...
                  className="inline-flex items-center justify-center px-8 py-3 rounded-full bg-[#111111] text-white text-sm font-bold hover:bg-[#FFD23F] hover:text-[#111111] transition-colors"
                >
                  Show more
                </button>
              </div>
